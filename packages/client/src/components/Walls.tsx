...
import sideBar from '../images/-e-Sidebar.png';
...
const Walls = () => {
    ...
    useEffect(() => {
        ...
        if (left_sidebar && left_sidebar.current) left_sidebar.current.style.left = `${((window.innerWidth - 1680) / 2) -115}px`;
        if (right_sidebar && right_sidebar.current) right_sidebar.current.style.right = `${((window.innerWidth - 1680) / 2)-150 }px`;
