...
import NEWSHOLDER from "../../images/mobile/News-frame.png";
...
function SliderItem(props: any) {
  ...
  return (
    <li
      className={cx({
        "news-mobile__slide-li": 1,
        "slide--current": current === index,
        "slide--previous": current - 1 === index,
        "slide--next": current + 1 === index,
      })}
    >
      <div className="news-mobile__slider-wrapper">
        <span className="news-mobile__slider-wrapper__content">{src}</span>
        <img
          className="news-mobile__slider-wrapper__holder"
          src={link ? NewsReadMore : NEWSHOLDER}
          alt=""
        />
        {link && (
          <a
            href={link}
            className="news-mobile__slider-wrapper__link"
            target="_blank"
          >
            More
          </a>
...
function Slider({ currentData, slides }: any) {
  const wrapperTransform = {
    transform: `translateX(${
      -1 * (currentData * (100 / slides.length)) + 0.5
    }%)`,
  };
...
export default function News({ slideData }: any) {
  ...
  return (
    <div className="news-mobile">
      <img
        src={Arrow}
        className="news-mobile__navbar_right"
        alt="navbar"
        onClick={handleNextClick(1)}
      />
      <img
        src={Arrow}
        className="news-mobile__navbar_left"
        alt="navbar"
        onClick={handleNextClick(-1)}
      />
