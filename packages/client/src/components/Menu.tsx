...
const Menu = ({ scrollToSlide, getCurrentSlideIndex, slidesCount, forwardRef, pages }: { pages: any, forwardRef: any, scrollToSlide: (pageNumber: number) => void, getCurrentSlideIndex: () => number, slidesCount: number }) => {
    ...
    return (
        <div className="tabs">
            ...
            {pages.map((page: any, index: number) => <img src={page.menuImage} className={`tabs-${index}`} style={{ transform: currentSlideIndex === index ? `translateX(-30px)` : `translateX(0)` }} onClick={() => scrollToSlide(index)} alt="" />)}
